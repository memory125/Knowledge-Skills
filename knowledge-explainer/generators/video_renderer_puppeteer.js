 ...
 */
async function generateFrame(scene, progress, width, height, outputPath, page) {
...
async function renderVideo(config) {
    ...
    const framesDir = path_1.default.join(outputDir, 'frames');
    ...
    try {
        ...
        for (const scene of scenes) {
            ...
            for (let i = 0; i < totalFrames; i++) {
                ...
                const outputPath = path_1.default.join(framesDir, `frame_${String(frameIndex).padStart(6, '0')}.png`);
        ...
        const outputPath = path_1.default.join(outputDir, 'output.mp4');
 ...
 * Stitch frames into video using FFmpeg
 */
async function stitchVideo(framesDir, outputPath, fps) {
    return new Promise((resolve, reject) => {
        const { exec } = require('child_process');
        const framePattern = path_1.default.resolve(framesDir, 'frame_%06d.png');
